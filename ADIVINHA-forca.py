...
secreto = 'perfume'  # a cada rodada do laço a pessoa digita uma letra pra tentar advinhar a palavra completa
...
while True:
    ...
    for letra_secreta in secreto:
        if letra_secreta in digitadas:      # se a letra_secreta estiver dentro das letras que a pessoa digitou, ele vai
            secreto_temporario += letra_secreta  # add nessse secreto_temporio a letra_secreta (add a letra)
        else:
            secreto_temporario += '*'  # é a palavra que ainda nao tem todo significado ex: **r**** = perfume
